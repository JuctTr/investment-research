...
import { StatusRepository } from '@/modules/xueqiu/repositories/status.repository';
...
export class StatusCrawlerService {
  ...
  async fetchUserStatuses(
    ...
    try {
      ...
      if (!data.statuses || data.statuses.length === 0) {
        ...
        return { statuses: [], hasMore: false, nextMax: undefined, nextId: undefined };
      }

      // 转换并保存数据
      const convertedStatuses = await this.convertAndSave(data.statuses);

      this.logger.log(`成功抓取用户 ${userId} 的 ${convertedStatuses.length} 条动态`);
    ...
    try {
      ...
      const result = await browserPage.evaluate(
        async ({ userId, page, maxId, type, apiBase }) => {
          ...
          const response = await fetch(`${apiBase}/v4/statuses/user_timeline.json?${params.toString()}`, {
            method: 'GET',
            credentials: 'include',
            headers: {
  ...
  private sanitizePrismaObjects(obj: any): any {
    ...
    if (typeof obj === 'object' && !Array.isArray(obj) && obj.$type === 'DateTime' && obj.value !== undefined) {
  ...
  private processPictures(pic: string | undefined): string[] {
    ...
    try {
      // pic 字段通常是逗号分隔的字符串
      const picArray = pic.split(',').map(p => p.trim()).filter(Boolean);
  ...
  private async convertAndSave(statuses: StatusResponse['statuses']) {
    ...
    for (const status of statuses) {
      ...
      this.logger.debug(`created_at type: ${typeof cleanStatus.created_at}, value: ${JSON.stringify(cleanStatus.created_at)}`);
      ...
      if (typeof timestamp === 'number') {
        ...
        if (timestamp < 1000000000000) {
          ...
          this.logger.debug(`Converted from seconds: ${timestamp} -> ${createdAt.toISOString()}`);
        } else {
          // 毫秒级时间戳，直接使用
          createdAt = new Date(timestamp);
          this.logger.debug(`Converted from milliseconds: ${timestamp} -> ${createdAt.toISOString()}`);
        }
      } else if (typeof timestamp === 'string') {
        // 字符串格式的日期（包括从 Prisma DateTime 清理后的字符串）
        createdAt = new Date(timestamp);
        this.logger.debug(`Converted from string: ${timestamp} -> ${createdAt.toISOString()}`);
      } else if (timestamp instanceof Date) {
        // 已经是 Date 对象
        createdAt = new Date(timestamp.getTime());
        this.logger.debug(`Converted from Date: ${timestamp.toISOString()} -> ${createdAt.toISOString()}`);
      } else {
        // 默认使用当前时间
        createdAt = new Date();
        this.logger.debug(`Using default current time: ${createdAt.toISOString()}`);
      }

      // 验证日期是否有效
      if (isNaN(createdAt.getTime())) {
        createdAt = new Date();
        this.logger.debug(`Invalid date, using current time: ${createdAt.toISOString()}`);
      }

      // 最后确保 createdAt 是一个纯 JavaScript Date 对象
      if (!(createdAt instanceof Date) || isNaN(createdAt.getTime())) {
        createdAt = new Date();
        this.logger.debug(`createdAt is not a valid Date, using current time: ${createdAt.toISOString()}`);
      }
      ...
      this.logger.debug(`Final createdAt for status ${cleanStatus.id}: ${createdAt.toISOString()}, type: ${typeof createdAt}, constructor: ${createdAt.constructor.name}`);
   ...
   * 获取数据库中的用户动态
   */
  async getStoredStatuses(userId: string, options: { page?: number; pageSize?: number } = {}) {
